...
builtinHead (SExpr xs)
    ...
    | (Str xs')   <- head xs           = return $ Str [head xs']
...
builtinTail (SExpr xs)
    ...
    | (Str xs')   <- head xs           = return $ Str $ tail xs'
...
builtinJoin (SExpr xs)
    ...
    | (Str _) <- head xs   = return $ Str $ foldl (\acc (Str xs') -> acc ++ xs') [] xs
    | otherwise            = return $ Err $ "join called with wrong arguments, got " ++ show xs
